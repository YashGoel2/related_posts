...
# This script fetches all the interview experience articles sequentially, checks if the article contains the sub-string supplied by the caller, removes the sub-string from the article and stores it back.
...
def main():
	...
	query = "SELECT table_key FROM all_posts WHERE topic = 19 LIMIT %d" % int(limit)
